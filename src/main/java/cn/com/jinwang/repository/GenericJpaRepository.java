...
public abstract class GenericJpaRepository<T, ID extends Serializable>
	...
	private static final String PERSISTENCE_UNIT_NAME = "wmailboxJpaUnit";
	...
	public Optional<T> findById(ID id) {
		final T result = emf.createEntityManager().find(persistentClass, id);
		return Optional.fromNullable(result);
	}

	@Override
	public Optional<T> findById(Optional<ID> idOp) {
		if (idOp.isPresent()) {
			final T result = emf.createEntityManager().find(persistentClass,
					idOp.get());
			return Optional.fromNullable(result);
		} else {
	...
	public List<T> findAll() {
		...
		cq.select(entities);
		TypedQuery<T> q = emf.createEntityManager().createQuery(cq);
	...
	public long countAll() {
		...
		TypedQuery<Long> q = emf.createEntityManager().createQuery(qs,
				Long.class);
	...
	public void delete(T entity) {
		emf.createEntityManager().remove(entity);
	}

	@Override
	public void deleteById(Long id) {
		T entity = emf.createEntityManager().find(getEntityClass(), id);
		delete(entity);
	}
	...
	public void deleteById(Optional<Long> id) {
		if (id.isPresent()) {
			T entity = emf.createEntityManager().find(getEntityClass(),
					id.get());
			delete(entity);
		}
	}
...
//		}
	...
	public List<T> findAll(int startRow, int endRow, Optional<SortBy> sortBy) {
		String qs = "select p from " + getEntityClass().getSimpleName()
				+ " as p order by p.%s %s";
		SortBy sb = getSortByOrDefault(sortBy);
		qs = String.format(qs, sb.getField(), sb.getDirection());
		TypedQuery<T> q = emf.createEntityManager().createQuery(qs,
				getEntityClass());
	...
	public List<T> findAllTop(int startRow, int endRow, Optional<SortBy> sortBy) {
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from " + getEntityClass().getSimpleName()
				+ " as p where p.parent is NULL order by p." + sb.getField()
				+ " " + sb.getDirection();
		TypedQuery<T> q = emf.createEntityManager().createQuery(qs,
				getEntityClass());
		q.setFirstResult(startRow);
	...
	public long countAllTop() {
		String qs = "select count(distinct p) from "
	...
	public List<T> findAllTopMine(LocalUser user, int startRow, int endRow,
			Optional<SortBy> sortBy) {
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from "
				+ getEntityClass().getSimpleName()
				+ " as p where p.parent is NULL and p.creator = :creator order by p."
	...
	public List<T> findAllTopOthers(LocalUser user, int startRow, int endRow,
			Optional<SortBy> sortBy) {
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from "
				+ getEntityClass().getSimpleName()
	...
	public List<T> findAllMine(LocalUser user, int startRow, int endRow,
		...
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from " + getEntityClass().getSimpleName()
				+ " as p where p.creator = :creator order by p."
				+ sb.getField() + " " + sb.getDirection();
		TypedQuery<T> q = emf.createEntityManager().createQuery(qs,
				getEntityClass());
		q.setParameter("creator", user);
	...
	public List<T> findAllOthers(LocalUser user, int startRow, int endRow,
		...
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from " + getEntityClass().getSimpleName()
				+ " as p where :user member of p.sharedUsers order by p."
				+ sb.getField() + " " + sb.getDirection();
		TypedQuery<T> q = emf.createEntityManager().createQuery(qs,
				getEntityClass());
		q.setParameter("user", user);
	...
	public List<T> children(T parent, int startRow, int endRow,
			Optional<SortBy> sortBy) {
		SortBy sb = getSortByOrDefault(sortBy);
		String qs = "select p from " + getEntityClass().getSimpleName()
	...
	public List<T> findAllWithQstr(int startRow, int endRow,
		...
		TypedQuery<T> q = emf.createEntityManager().createQuery(sbd.toString(),
				getEntityClass());
		q.setFirstResult(startRow);
		q.setMaxResults(endRow - startRow);
		return q.getResultList();
	...
	public long countAllWithQstr(String whereLikeString) {
		...
		sbd.append(" as p where ");
		sbd.append(whereLikeString);
		TypedQuery<Long> q = emf.createEntityManager().createQuery(
				sbd.toString(), Long.class);
		return q.getSingleResult();
	}

	public List<T> findAllMineWithQstr(LocalUser user, int startRow,
			int endRow, Optional<SortBy> sortBy, String whereLikeString) {
		SortBy sb = getSortByOrDefault(sortBy);
		StringBuilder sbd = new StringBuilder("select p from ");
		sbd.append(getEntityClass().getSimpleName());
	...
	public List<T> findAllOthersWithQstr(LocalUser user, int startRow,
			int endRow, Optional<SortBy> sortBy, String whereLikeString) {
		SortBy sb = getSortByOrDefault(sortBy);
		StringBuilder sbd = new StringBuilder("select p from ");
		sbd.append(getEntityClass().getSimpleName());
	...
	public Optional<T> findOneByOneField(String fieldName, String fieldValue) {
		try {
			...
			sb.append(" = :");
			sb.append(fieldName);
			TypedQuery<T> q = emf.createEntityManager().createQuery(
					sb.toString(), getEntityClass());
	...
	public List<T> findAllByOneField(String fieldName, String fieldValue) {
		...
		sb.append(" = :");
		sb.append(fieldName);
		TypedQuery<T> q = emf.createEntityManager().createQuery(sb.toString(),
				getEntityClass());
		q.setParameter(fieldName, fieldValue);
		return q.getResultList();
	}
	...
	public List<T> breadcrumb(Long domainId) {
		T d = findById((ID) domainId).get();
		String parentIds = ((BaseTreeDomain) d).getParentIds();
	...
	public List<T> amIInSharedUsers(LocalUser user) {
		String qs = "select w from " + getEntityClass().getSimpleName()
	...
	@Override
	public List<T> findAllMineAndOthers(LocalUser user, int startRow,
			int endRow, Optional<SortBy> sortBy, Optional<String> qstr) {
		return null;
	}
